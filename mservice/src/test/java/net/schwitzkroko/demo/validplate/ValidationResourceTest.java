...
class ValidationResourceTest {
    ...
    void tesWithTestCasesOriginal(PlateTestData tc) {
        log.debug("validate: input='{}' expected success={} output='{}'",
    ...
    // @Disabled("Custom district test cases are currently not supported by the implementation")
    @ParameterizedTest(name = "[{index}] {0}")
    ...
    // @Disabled("Custom special test cases are currently not supported by the implementation")
    @ParameterizedTest(name = "[{index}] {0}")
